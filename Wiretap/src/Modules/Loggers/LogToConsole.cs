...
public class LogToConsole : ILog
{
    ...
    public ConsoleStyle DebugStyle { get; set; } = new(ConsoleColor.Black, ConsoleColor.Gray);
    ...
    public void Invoke(TraceContext context, LogAction next)
    {
        ...
        {
            using (Styles.TryGetValue(styleName, out var style) ? style.Apply() : DebugStyle.Apply())
            {
