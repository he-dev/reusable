...
namespace Reusable.Tests.XUnit.IOnymous
{
    public class CompositeProviderTest
    {
        ...
        public async Task Can_get_resource_by_default_name()
        {
            ...
            var resource = await composite.GetAsync("blub:x/123", ImmutableSession.Empty.Set(Use<IProviderSession>.Namespace, x => x.DefaultName, "InMemoryProvider"));
        ...
        public async Task Can_get_resource_by_custom_name()
        {
            var composite = new CompositeProvider(new IResourceProvider[]
            {
                ...
                new InMemoryProvider(new UriStringToSettingIdentifierConverter(), new[] { ResourceProvider.DefaultScheme }, ImmutableSession.Empty.Set(Use<IProviderSession>.Namespace, x => x.CustomName, "blub"))
                {
                    //{ "x.123", "blub2" },
                    { "x.123", "blub3" }
                },
            ...
            var resource = await composite.GetAsync("blub:x/123", ImmutableSession.Empty.Set(Use<IProviderSession>.Namespace,x => x.CustomName, "blub"));
