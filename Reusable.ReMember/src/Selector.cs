...
public class Selector : IEnumerable<SelectorToken>
{
    ...
    public Selector(LambdaExpression expression, params ISelectorTokenFactoryParameter[] parameters)
    {
        ...
        var selectorTokenFactoryProvider = Metadata.Member.SelectorTokenFactory();
        
        _tokens =
            from f in selectorTokenFactoryProvider.GetSelectorTokenFactories(Metadata.Member)
        ...
        _formatter = Metadata.Member.SelectorFormatter();
    }
...
{
    // Gets the first selector formatter or default.
    public static ISelectorFormatter SelectorFormatter(this MemberInfo member)
    {
        foreach (var m in member.Ancestors())
        {
    ...
    // Gets the first token factory or default.
    public static ISelectorTokenFactoryProvider SelectorTokenFactory(this MemberInfo member)
    {
        foreach (var m in member.Ancestors())
        {
