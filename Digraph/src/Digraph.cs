...
public static class Digraph
{
    ...
    public static IEnumerable<T> TopologicalSort<T>(this IEnumerable<(T U, T V)> source, IEqualityComparer<T> comparer)
    {
        ...
        if (edges.Any())
        {
            throw new DirectedCycleException($"Detected one or more cycles between the following nodes: [{string.Join(", ", edges)}]");
        }
    }
}

public class DirectedCycleException : Exception
{
    public DirectedCycleException(string message) : base(message) { }
}