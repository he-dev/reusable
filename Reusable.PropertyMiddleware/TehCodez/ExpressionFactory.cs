...
{
    internal abstract class ExpressionFactory_
    {

    }

    internal static class GetterExpresssionFactory
    {
        public static Expression<Func<object, object>> CreateMemberExpression(
            Type type,
            string propertyName)
        {
            var instance = Expression.Parameter(type, "obj");
            var property = Expression.Property(instance, propertyName);
            return Expression.Lambda<Func<object, object>>(property, instance);
        }

        public static Expression<TFunc> CreateIndexExpression<TFunc>(
            Type type,
            PropertyInfo propertyInfo,
            params ParameterInfo[] parameters)
        {
            //var getterDelegate = propertyInfo.GetGetMethod();
            var instance = Expression.Parameter(typeof(object), "instance");
            var arguments =
                parameters
                    .Select((a, i) => Expression.Parameter(a.ParameterType, $"Index{i}"))
                    .ToArray();
            var index = Expression.Property(Expression.Convert(instance, type), propertyInfo, arguments.Cast<Expression>());
            //var index = Expression.Call(instance, propertyInfo.GetGetMethod(), arguments.Cast<Expression>());
            var result = Expression.Lambda(
                Expression.Convert(index, typeof(object)),
                new[] { instance }.Concat(parameters
                    .Select((a, i) => Expression.Parameter(typeof(object), $"Index{i}"))));
            //var result = Expression.Lambda(index, new[] { instance }.Concat(arguments));

            return (Expression<TFunc>)result;
        }
