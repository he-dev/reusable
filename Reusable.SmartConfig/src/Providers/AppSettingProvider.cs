...
namespace Reusable.SmartConfig
{
    public class AppSettingProvider : SettingProvider
    {
        ...
        protected override Task<IResourceInfo> GetAsyncInternal(UriString uri, IImmutableSession metadata)
        {
            ...
            return Task.FromResult<IResourceInfo>(new AppSettingInfo(uri, element?.Value, ImmutableSession.Empty.Set(Use<IResourceSession>.Scope,x => x.ActualName, settingIdentifier)));
        }
    ...
    internal class AppSettingInfo : ResourceInfo
    {
        ...
        internal AppSettingInfo([NotNull] UriString uri, [CanBeNull] string value, IImmutableSession metadata)
            : base(uri, metadata.Set(Use<IResourceSession>.Scope,x => x.Format, MimeType.Text))
        {
            _value = value;
        }
