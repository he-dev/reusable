...
namespace Reusable.Translucent.Middleware
{
    public class ResourceMiddlewareTest
    {
        ...
        {
            var c1 = Mock.Create<TestFileController>(Constructor.NotMocked);
            var c2 = Mock.Create<TestFileController>(Constructor.NotMocked);
            var c3 = Mock.Create<TestFileController>(Constructor.NotMocked);
        ...
        {
            var c1 = Mock.Create<TestFileController>(Behavior.CallOriginal, "a");
            var c2 = Mock.Create<TestFileController>(Behavior.CallOriginal, "b");
            var c3 = Mock.Create<TestFileController>(Behavior.CallOriginal, "d");
            ...
            await resources.InvokeAsync(new ResourceContext { Request = Request.CreateGet<FileRequest>("file:///foo").Pipe(r => { r.ControllerId = "b"; }) });
        ...
        {
            var c1 = Mock.Create<TestFileController>(Behavior.CallOriginal, "a");
            var c2 = Mock.Create<TestHttpController>(Behavior.CallOriginal, "b");
            var c3 = Mock.Create<TestFileController>(Behavior.CallOriginal, "d");
        ...
        public async Task Can_filter_controllers_by_tag()
        {
            ...
            {
                Request = Request.CreateGet<FileRequest>("file:///foo").Pipe(r => { r.ControllerTags.Add("b"); })
            });
    ...
    {
        public TestFileController(string? id) : base(id, "file") { }
    ...
    {
        public TestHttpController(string? id) : base(id, "http") { }
