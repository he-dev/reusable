...
namespace Reusable.IOnymous
{
    public class EmbeddedFileProvider : FileProvider
    {
        ...
        public EmbeddedFileProvider([NotNull] Assembly assembly, IImmutableSession metadata = default)
            : base((metadata ?? ImmutableSession.Empty).Set(Use<IProviderSession>.Namespace, x => x.AllowRelativeUri, true))
        {
        ...
        protected override Task<IResourceInfo> GetAsyncInternal(UriString uri, IImmutableSession metadata)
        {
            ...
            return Task.FromResult<IResourceInfo>(new EmbeddedFileInfo(fullUri, metadata.Get(Use<IResourceSession>.Namespace, y => y.Format), getManifestResourceStream));
        }
