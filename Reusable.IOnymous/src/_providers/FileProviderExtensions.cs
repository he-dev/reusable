...
namespace Reusable.IOnymous
{
    public static class FileProviderExtensions
    {
        ...
        {
            return await resourceProvider.GetAsync(CreateUri(path), metadata.ThisOrEmpty().Set(Use<IResourceSession>.Namespace, x => x.Format, format));
        }
        ...
        {
            using (var stream = await ResourceHelper.SerializeTextAsync(value, metadata.ThisOrEmpty().Get(Use<IAnySession>.Namespace, x => x.Encoding, Encoding.UTF8)))
            {
                return await resourceProvider.PutAsync(CreateUri(path), stream, metadata.ThisOrEmpty().Set(Use<IResourceSession>.Namespace, x => x.Format, MimeType.Text));
            }
        }
        ...
        {
            return await resourceProvider.DeleteAsync(CreateUri(path), metadata.ThisOrEmpty().Set(Use<IResourceSession>.Namespace, x => x.Format, MimeType.Text));
        }
