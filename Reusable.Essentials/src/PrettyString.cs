...
public interface IPrettyString
{
    ...
    string Render(Type type, bool includeNamespace = false);

    [ContractAnnotation("methodInfo: null => halt")]
    string Render(MethodInfo methodInfo, bool includeNamespace = false);
}

public class PrettyString : IPrettyString
{
    private readonly ConcurrentDictionary<Type, string> _prettyStrings = new();
