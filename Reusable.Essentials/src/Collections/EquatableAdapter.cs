...
internal class EquatableAdapter<T> : IEquatable<T>
{
    ...
    public EquatableAdapter([CanBeNull] T obj, IEqualityComparer<T> comparer)
    {
        _obj = obj;
        _comparer = comparer;
    }
...
{
    public static IEquatable<T> AsEquatable<T>([CanBeNull] T obj, Func<T, T, bool> equals, Func<T, int> getHashCode)
    {
    ...
    public static IEquatable<T> AsEquatable<T>([CanBeNull] T obj, Func<T, T, bool> equals)
    {
        if (equals == null) throw new ArgumentNullException(nameof(equals));
