...
namespace Reusable.Htmlize
{
    ...
    {
        string Render(IHtmlElement markupElement);
    }
    ...
    public abstract class MarkupRenderer : IMarkupRenderer
    {
        ...
        private MarkupFormattingOptions GetFormatting(string name) => _formatting.TryGetValue(name, out var options) ? options : MarkupFormattingOptions.None;

        #region IMarkupRenderer

        public string Render(IHtmlElement markupElement)
        {
            var content = (markupElement ?? throw new ArgumentNullException(nameof(markupElement))).Aggregate(
                    new StringBuilder(),
                    (result, next) => result.Append(
                        next is IHtmlElement e
                            ? Render(e)
