...
public class SmtpController : MailToController<SmtpRequest>
{
    ...
    public override async Task<Response> CreateAsync(SmtpRequest request)
    {
        ...
        if (request is SmtpRequest.Text && request.Body.Peek() is string text)
        {
        ...
        if (request is SmtpRequest.Stream && request.Body.Peek() is Stream stream)
        {
            multipart.Add(new TextPart(request.IsHtml ? TextFormat.Html : TextFormat.Plain)
            {
        ...
        return Success<Translucent.Data.SmtpResponse>(request.ResourceName);
    }
