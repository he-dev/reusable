...
namespace Reusable.Extensions
{
    ...
    {
        public static IEnumerable<(Exception Exception, int Depth)> Flatten(this Exception exception)
        {
        ...
        public static IEnumerable<(string Caller, string FileName, int LineNumber)> GetStackTrace(this Exception exception)
        {
            ...
            foreach (var stackFrame in stackFrames)
            {
                yield return
                (
                    Caller: (stackFrame.GetMethod() as MethodInfo)?.ToPrettyString() ?? string.Empty,
