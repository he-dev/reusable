...
namespace Reusable
{
    ...
    public static class BinaryOperation
    {
        ...
        {
            return (Func<T, T, TResult>)Cache.GetOrAdd(typeof(T), t => CreateBinaryFunc<T, TResult>(expressionFunc));
        }

        private static Func<T, T, TResult> CreateBinaryFunc<T, TResult>(Func<Expression, Expression, BinaryExpression> binaryExpression)
        {
            ...
            return Expression.Lambda<Func<T, T, TResult>>(binaryOperation, leftParameter, rightParameter).Compile();
        }
    }
}
