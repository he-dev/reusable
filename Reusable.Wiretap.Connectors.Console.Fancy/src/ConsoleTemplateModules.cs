...
public static class ConsoleTemplateModules
{
    ...
    public static Action<ILogEntry> Property<T>(this Action<ILogEntry> previous, object fallback) where T : ILogProperty
    {
        return previous.Then(entry =>
        {
            var value = entry.GetPropertyOrDefault<T>() ?? fallback;
    ...
    {
        return entry.TryGetProperty<ConsoleProperty.Template, IHtmlElement>(out var template) ? template : template;
    }
}