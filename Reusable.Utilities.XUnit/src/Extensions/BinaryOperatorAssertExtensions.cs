...
namespace Reusable.Utilities.Xunit.Extensions
{
    ...
    public static class BinaryOperatorEqualityAssertExtensions
    {
        ...
        {
            BinaryOperator<IBinaryOperatorEqualityAssert>.Check(left, others, BinaryOperation<T>.Equal, Assert.True);
        }

        public static void IsFalse<T>(this IBinaryOperatorEqualityAssert assert, T left, params T[] others)
        {
            BinaryOperator<IBinaryOperatorEqualityAssert>.Check(left, others, BinaryOperation<T>.Equal, Assert.False);
        }
    }
    ...
    public static class BinaryOperatorInequalityAssertExtensions
    {
        ...
        {
            BinaryOperator<IBinaryOperatorInequalityAssert>.Check(left, others, BinaryOperation<T>.NotEqual, Assert.True);
        }

        public static void IsFalse<T>(this IBinaryOperatorInequalityAssert assert, T left, IEnumerable<T> others)
        {
            BinaryOperator<IBinaryOperatorInequalityAssert>.Check(left, others, BinaryOperation<T>.NotEqual, Assert.False);
        }
    }
