    ...
    public static class UnaryOperatorConvertAssertExtensions
    {
        public static void IsEqual<TSource, TExpected>(this IUnaryOperatorConvertAssert<TSource> assert, TExpected expected)
        {
            ...
            try
            {
                ...
                //$"Conversion from {typeof(TSource).Name.QuoteWith("'")} to {typeof(TExpected).Name.QuoteWith("'")} failed. Expected: {expected.Stringify()}, Actual: {assert.Stringify()}.");
            }
            catch (Exception ex)
            {
                throw DynamicException.Create("AssertFailed",$"Conversion from {typeof(TSource).Name.QuoteWith("'")} to {typeof(TExpected).Name.QuoteWith("'")} threw an exception: {ex}");
            }
        }
    }
}