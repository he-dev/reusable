...
namespace Reusable.Utilities.MSTest
{
    ...
    public static class CollectionAssertExtensions
    {
        ...
        {
            Assert.NotNull(collection, $"Collection of {typeof(T).Name.QuoteWith("'")} must not be null.");
            Assert.False(collection.Any(), $"Collection of {typeof(T).Name.QuoteWith("'")} must be empty.");
        }

        public static void IsNotEmpty<T>(this ICollectionAssert assert, [CanBeNull] IEnumerable<T> collection)
        {
            Assert.NotNull(collection, $"Collection of {typeof(T).Name.QuoteWith("'")} must not be null.");
            Assert.True(collection.Any(), $"Collection of {typeof(T).Name.QuoteWith("'")} must not be empty.");
        }
