...
public class ValidateSetting : ResourceNode
{
    public override async Task InvokeAsync(ResourceContext context)
    {
        if (context.Request is ConfigRequest config)
        {
            ...
            {
                if (context.Request.Data.TryPeek(out var newValue))
                {
                    Validate(config.ResourceName.Value, newValue, config.ValidationAttributes);
                }
                else
                {
                    throw DynamicException.Create("SettingValidation", $"Setting '{config.ResourceName.Value}' is required.");
                }
            }
            ...
            if (context.Request.Method == RequestMethod.Read)
            {
                ...
                else
                {
                    ...
                    {
                        Validate(config.ResourceName.Value, value, config.ValidationAttributes);
                    }
                }
            }
        }
