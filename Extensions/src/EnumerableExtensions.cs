...
public static class Enumerable2
{
    ...
    {
        using var ex = first.GetEnumerator();
        using var ey = second.GetEnumerator();

        var xm = false;
        var ym = false;
        while ((xm = ex.MoveNext()) | (ym = ey.MoveNext()))
        {
            yield return resultSelector
            (
                xm ? ex.Current : default,
                ym ? ey.Current : default
            );
        }
    }
    ...
    public static T SingleOrThrow<T>(this IEnumerable<T> source, string message)
    {
        ...
        try
        {
            ...
            throw new ArgumentException(message, paramName: nameof(source));
        }
        finally
        {
            if (e.MoveNext())
    ...
    public static IEnumerable<T> Parse<T>(this IEnumerable<Match> source, Func<string, Type, object>? convert = default)
    {
        ...
        foreach (var item in source)
        {
            var values =
                ...
                let v = g.Success ? g.Value : throw new ArgumentException($"{typeof(T)}'s property '{p.Name}' not found.", paramName: nameof(source))
