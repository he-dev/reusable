...
public static class ResourceHelpers
{
    ...
    {
        using var request = ConfigRequest.Json.Read(selector).Also(configure);
        using var response = await resource.InvokeAsync(request);
        return response.Body.Peek();
    }
    ...
    {
        using var request = ConfigRequest.Json.Write(selector, newValue).Also(configure);
