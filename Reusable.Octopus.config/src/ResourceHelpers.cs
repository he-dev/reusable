...
public static class ResourceHelpers
{
    ...
    {
        using var request = ConfigRequest.Create(selector, RequestMethod.Read).Also(configure);
        using var response = await resource.InvokeAsync(request);
        return response.Body.Value;
    }

    public static async Task WriteSettingAsync(this IResource resource, Selector selector, object newValue, Action<ConfigRequest>? configure = default)
    {
        using var request = ConfigRequest.Create(selector, RequestMethod.Update, newValue).Also(configure);
